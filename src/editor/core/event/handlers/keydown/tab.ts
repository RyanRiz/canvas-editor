...
export function tab(evt: KeyboardEvent, host: CanvasEvent) {
  ...
  //
  //   Tab      — insert an ElementType.TAB character. The layout engine
  //              (Draw.ts) advances to the next tab stop or the next
  //              default-tab-grid position when no explicit tab stops exist
  //              — matching Word's "Tab always inserts a tab character".
  //
  //   Shift+Tab — decrease the paragraph's left indent by one tab-width
  //               step (Word: "outdent"). If indent is already 0, no-op.
  //
