...
describe('Draw - incremental computeRowList (P2.2)', () => {
  ...
  it('page-geometry change renders bypass incremental resume and fall back to full layout', () => {
    ctx = createTestEditor({
      options: { pageMode: PageMode.CONTINUITY },
      data: { header: [], main: makeManyParagraphs(20), footer: [] }
    })
    ...
    const internal = draw as unknown as DrawInternals
    draw.markDirty(0, 10)
    const resumeSpy = vi.spyOn(internal, '_tryBuildResumeFrom')
  ...
  it('page-geometry change renders reuse cached intrinsic text metrics', () => {
    ctx = createTestEditor({
      options: { pageMode: PageMode.CONTINUITY },
      data: { header: [], main: makeManyParagraphs(20), footer: [] }
    })
    ...
    draw.render({ isCompute: true, isSubmitHistory: false })
    draw.markDirty(0, draw.getOriginalMainElementList().length - 1)
    const measureTextSpy = vi.spyOn(draw.textParticle, 'measureText')
    const measureBasisWordSpy = vi.spyOn(draw.textParticle, 'measureBasisWord')
    ...
    expect(measureBasisWordSpy).not.toHaveBeenCalled()
    measureTextSpy.mockRestore()
    measureBasisWordSpy.mockRestore()
  })

  it('list-free geometry-change renders skip list width recomputation', () => {
    ctx = createTestEditor({
      options: { pageMode: PageMode.CONTINUITY },
      data: { header: [], main: makeManyParagraphs(20), footer: [] }
    })
    ...
    draw.render({ isCompute: true, isSubmitHistory: false })
    draw.markDirty(0, draw.getOriginalMainElementList().length - 1)
    const listStyleSpy = vi.spyOn(draw.listParticle, 'computeListStyle')
    ...
    expect(listStyleSpy).not.toHaveBeenCalled()
    listStyleSpy.mockRestore()
  })
